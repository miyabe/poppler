...
bool StreamPredictor::getNextLine() {
  ...
  if (predictor == 2) {
    if (nBits == 1) {
	  ...
	  c = predLine[i];
	  predLine[i] = 0;
      predLine[i] |= (unsigned char)((d ^= (c >> 7) & 1) << 7);
      predLine[i] |= (unsigned char)((d ^= (c >> 6) & 1) << 6);
      predLine[i] |= (unsigned char)((d ^= (c >> 5) & 1) << 5);
      predLine[i] |= (unsigned char)((d ^= (c >> 4) & 1) << 4);
      predLine[i] |= (unsigned char)((d ^= (c >> 3) & 1) << 3);
      predLine[i] |= (unsigned char)((d ^= (c >> 2) & 1) << 2);
      predLine[i] |= (unsigned char)((d ^= (c >> 1) & 1) << 1);
      predLine[i] |= (unsigned char)(d ^= c & 1);
      }
    } else if (nBits == 8) {
      for (i = pixBytes; i < rowBytes; ++i) {
